function Invoke-ReflectionObfuscated{
    ...
    $Description = "Este bypass es una versión ofuscada del bypass por reflexión de Matt Graebers. Consiste en usar la reflexión de .NET para cambiar el valor 4ms1InitFail a true, lo que hace que AMSI no se cargue correctamente en la sesión de powershell activa. Para la ofuscación se utilizarón varias de las tecnicas de Invoke-Obfuscation"
    $Script = {
